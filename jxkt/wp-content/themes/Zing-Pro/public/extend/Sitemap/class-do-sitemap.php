...
class DX_Seo_Sitemap_Do_Sitemap {	
	...
	function __construct() {
		add_action( 'wp_ajax_' . sitemap_xintheme . '_do_sitemap', array( $this, 'ajax_do_sitemap' ) );
		add_action( 'wp_ajax_' . sitemap_xintheme . '_delete_sitemap', array( $this, 'ajax_delete_sitemap' ) );
...
jQuery(document).ready(function($){
	$( '#generate-baidu' ).click(function(){
		...
		$.ajax({
			...
			type: 'POST',
			data: ({
					action: '<?php echo sitemap_xintheme; ?>_do_sitemap',
					nonce: '<?php echo wp_create_nonce( sitemap_xintheme . '_do_sitemap' ); ?>'
				}),
			cache: false,
	...
	$( '#delete-baidu' ).click(function(){
		...
		$.ajax({
			...
			type: 'POST',
			data: ({
					action: '<?php echo sitemap_xintheme; ?>_delete_sitemap',
					nonce: '<?php echo wp_create_nonce( sitemap_xintheme . '_delete_sitemap' ); ?>'
				}),
			cache: false,
			success: function(res){
				$( '#sitemap-progress' ).html( '' );
...
<?php	
	...
	function do_sitemap() {
		...
		if( $posts ) {
			while( $posts->have_posts() ) {
				...
				$xml .= '<loc>' . apply_filters( sitemap_xintheme . '_sitemap_post_link', $post_link ) . '</loc>';
		...
		if( $terms ) {
			foreach( $terms as $term ) {
				...
				$xml .= '<loc>' . apply_filters( sitemap_xintheme . '_sitemap_tax_link', $tax_link ) . '</loc>';
	...
	function ajax_do_sitemap() {
		if( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], sitemap_xintheme . '_do_sitemap' ) )
			die( 'ERROR!' );
		$this->do_xml();
		die();
	}
	...
	function ajax_delete_sitemap() {
		if( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], sitemap_xintheme . '_delete_sitemap' ) )
			die( 'ERROR!' );
		$file = $this->get_sitemap_file();
		if( file_exists( $file ) ) {
			@unlink( $file );
