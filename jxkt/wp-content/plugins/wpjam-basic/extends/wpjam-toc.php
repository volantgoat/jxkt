...
class WPJAM_Toc{
	...
	public function get_setting($name){
		if(!is_null($this->settings)){
			...
			return wpjam_basic_get_setting('toc_'.$name);
		}
	}
...
function wpjam_toc_get_setting($name){
	return WPJAM_Toc::get_instance()->get_setting($name);
}
...
$toc_instance	= WPJAM_Toc::get_instance();

add_shortcode('toc',		[$toc_instance, 'shortcode_callback']);
add_filter('the_content',	[$toc_instance, 'filter_content']);
add_action('wp_head', 		[$toc_instance, 'on_head']);
...
if(is_admin()){
	...
	add_action('wpjam_plugin_page_load', function($plugin_page, $current_tab){
		if($plugin_page == 'wpjam-posts'){
			...
			if($current_tab == 'toc'){
				wpjam_register_option('wpjam-toc', ['WPJAM_Toc', 'get_option_setting']);
			}
		}
	}, 10, 2);
}
