<?php
add_filter('wpjam_server_status_tabs',function($tabs){
	$tabs	= [];
	...
	if(strtoupper(substr(PHP_OS,0,3)) !== 'WIN'){
		$tabs['server']	= [
			...
			'widgets'	=>[
				'server'	=> ['title'=>'服务器信息'],
				'version'	=> ['title'=>'服务器版本',	'context'=>'side'],
				'php'		=> ['title'=>'PHP扩展'],
			]
		];
	}
	...
	if(function_exists('opcache_get_status')){
		$tabs['opcache']	= [
			...
			'function'	=>'dashboard',
			'widgets'	=>[
				'opcache-usage'			=> ['title'=>'OPCache使用率'],
				'opcache-status'		=> ['title'=>'OPCache状态',		'context'=>'side'],
				'opcache-configuration'	=> ['title'=>'OPCache配置信息',	'context'=>'side']
			]
		];
	}
	...
	if(method_exists('WP_Object_Cache', 'get_mc')){
		$tabs['memcached']	= [
			...
			'function'	=>'dashboard',
			'widgets'	=>[
				'memcached-usage'	=> ['title'=>'Memcached使用率'],
				'memcached-status'	=> ['title'=>'Memcached状态',	'context'=>'side']
			]
		];
	}
